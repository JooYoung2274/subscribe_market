...
export class SubscribeRepository {
  constructor(
    ...
    private subscribeRepository: Repository<SubscribeList>,
  ...
  async updateSubscribeAndAlarmBySubscribeId({
    ...
    const isSubscribe = await this.subscribeRepository.findOne({
      where: {
        id: subscribeId,
      },
    });
    ...
    const isAlarm = await this.alarmRepository.findOne({
      where: {
        SubscribeListId: subscribeId,
      },
    });
  ...
  async findSubscribeList(oauthId: number) {
    return await this.subscribeRepository.find({ where: { UserId: oauthId } });
  }
}
